...
const containsAnd = (arr) => {
  let res = arr.filter(n=>n.includes("and"));
  return res;
};
/* ------------------------------------------------------------------------------------------------
CHALLENGE 4
...
const notInFirstArray = (forbiddenValues, arr) => {
  let res = arr.filter(v=>!forbiddenValues.includes(v)); //this line screws up the test for some reason
  return res;
}
...
const getBaseStatGreaterThan = (arr, minBaseStat) => {
  let newArr = arr.filter(el => el.baseStat > minBaseStat)
...
const getStatName = (arr, minBaseStat) => {
  let x = arr.filter(el => el.baseStat > minBaseStat)
  let z = arr.filter(el => el.baseStat > minBaseStat).map(el=>el.stat.name)
...
const evenOddNumericValues = (arr) => {
  return  arr.filter(el => el===Number(el)).map(el=>el%2==0 ? "even":"odd")
};
