...
const toLastNames = people => {
  let x = people.map(p => p.firstName + " " + p.lastName)
  return x;
};
/* ------------------------------------------------------------------------------------------------
CHALLENGE 2
...
const addValues = (arr) => {
  let x = arr.reduce((acc, cur) => acc + cur, 0);
  return x;
};
...
const addPurchases = (arr) => {
  let res = arr.reduce((acc, cur) => acc + cur.purchasePrice, 0);
  return res;
};
...
const returnNames = (arr) => {
  let res = []
  arr.reduce((acc, cur) => res.push(cur.name), 0)
  return res
};
...
const countNumberOfChildren = (arr) => {
  ...
  return arr.reduce((acc, cur) => {
    // console.log(cur)
    return cur.children ? acc + cur.children.length : acc
  }, 0
  )
}
