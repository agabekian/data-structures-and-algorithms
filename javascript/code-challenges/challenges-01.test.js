...
const allUpperCase = (arr) => {
  let res = [];
  arr.forEach(el => res.push(el.toUpperCase()))
  return res;
};
...
const greeting = (word) => {
  word = word.split("")
  word.forEach(x => console.log(x))
  let res = [];
  word.forEach(el => res.push(el.toUpperCase()))
  return res.join("") + "!";
};
console.log(greeting("sssss"));
const speaker = (words, callback) => {
  let res = []
  words.forEach(w => res.push(callback(w)))
  return res
};
...
const createList = (availableItems) => {
  let list = [];
  availableItems.forEach(p=>list.push(p))
  return list;
};
...
const fizzbuzz = (arr) => {
  let res = []
    for(let x of arr){
      ...
      else if(x % 5==0) res.push("Buzz");
      else if(x % 3==0) res.push("Fizz");
      else res.push(x)
    }
    return res;
};
