...
const raisedToTheThird = (arr) => {
  let res = [];
  arr.forEach(n => res.push(Math.pow(n, 3)))
  return res;
};
...
Write a function named forLoopTwoToThe that, given
 an array of integers as input, iterates over the array and returns a new array. 
...
const evenOdd = (arr) => {
  let res = [];
  arr.map(n => {
      if (isNaN(n)) {res.push("N/A")}
      else {
          if (n % 2 == 0) {res.push("even")}
          if (n % 2 != 0) {res.push("odd")}
      }
  });
  return res;
};
